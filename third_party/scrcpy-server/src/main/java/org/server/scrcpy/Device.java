...
public final class Device {
    ...
    private ScreenInfo computeScreenInfo(int maxSize) {
        ...
        if (displayInfo == null) {
            displayInfo = redroidDisplayInfo();
        }
        if (displayInfo == null) {
            throw new IllegalStateException("display service and ReDroid display properties are unavailable");
        }
    ...
    private static DisplayInfo redroidDisplayInfo() {
        try {
            int width = Integer.parseInt(Command.execReadOutput("getprop", "ro.boot.redroid_width").trim());
            int height = Integer.parseInt(Command.execReadOutput("getprop", "ro.boot.redroid_height").trim());
            if (width <= 0 || height <= 0) {
                return null;
            }
            return new DisplayInfo(new Size(width, height), 0);
